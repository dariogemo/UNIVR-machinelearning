...
def smote_oversampling(df):
    ...
    df_smote = pd.concat([df_no256_over, df[(df['anomaly'] == 2) | (df['anomaly'] == 5) | (df['anomaly'] == 6)]], axis=0)
...
def ctgan_oversampling(df : pd.DataFrame, discrete_cols : list) -> pd.DataFrame:
    ...
    for idx in anomaly_count.index:
        ...
        num_samples = int(anomaly_count.max() - anomaly_count.loc[idx] + 827) # check how many samples need to be generated
...
df = ctgan_oversampling(df_smote, ['typeofsteel_a300', 'outside_global_index', 'anomaly'])
